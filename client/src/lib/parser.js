export function extractComponents(xml, type, tag) {
  const results = []
  ...
  while ((m = re.exec(xml)) !== null) {
    ...
    results.push({ type, name, exported, inferredExported, perm, authorities, grantUri, taskAffinity, actions, schemes, hosts, mimeTypes })
  }
  return results
}
...
export function calcComponentRisk(c) {
  let score = 0
